    ...
    if (txnid) {
      const fetchTransactionStatus = async () => {
        try {
          ...
          // Autofill the user address and amount from the backend data
          if (response.data) {
            setUserAddress(response.data.walletAddress);
            setAmount(response.data.usdtAmount);
          }
        } catch (err) {
      ...
      const intervalId = setInterval(fetchTransactionStatus, 5000); // Retry every 5 seconds
